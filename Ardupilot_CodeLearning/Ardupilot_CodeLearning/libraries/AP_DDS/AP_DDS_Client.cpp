...
// Enable DDS at runtime by default 各主题的发布延迟时间设置
static constexpr uint8_t ENABLED_BY_DEFAULT = 1;     //如果值为`1`，表示系统默认启用DDS通信。
static constexpr uint16_t DELAY_TIME_TOPIC_MS = 10;   //时间同步主题的发布间隔为10毫秒，确保系统时间的高精度同步。
static constexpr uint16_t DELAY_BATTERY_STATE_TOPIC_MS = 1000; //电池状态每1000毫秒（1秒）发布一次，平衡了状态监控频率和通信负载。
#if AP_DDS_IMU_PUB_ENABLED
static constexpr uint16_t DELAY_IMU_TOPIC_MS = 5; //如果启用了IMU数据发布，该主题的发布间隔为5毫秒（200 Hz）。高频率IMU数据发布能为姿态估计和运动控制提供高精度支持，但也会占用较多的通信资源。
#endif // AP_DDS_IMU_PUB_ENABLED
static constexpr uint16_t DELAY_LOCAL_POSE_TOPIC_MS = 33; //本地位姿和速度数据的发布间隔为33毫秒（约30 Hz），适用于姿态控制或路径跟踪等需要平滑更新的场景。
static constexpr uint16_t DELAY_LOCAL_VELOCITY_TOPIC_MS = 33;
static constexpr uint16_t DELAY_GEO_POSE_TOPIC_MS = 33; //地理位姿（例如GPS位置数据）以33毫秒为间隔发布。
static constexpr uint16_t DELAY_CLOCK_TOPIC_MS = 10; //时钟主题同步频率为10毫秒，确保各系统模块之间的时间一致性。
static constexpr uint16_t DELAY_GPS_GLOBAL_ORIGIN_TOPIC_MS = 1000; //GPS原点每1000毫秒（1秒）发布一次。原点数据通常在全球坐标系统中保持相对不变，因此无需频繁发布。
static constexpr uint16_t DELAY_PING_MS = 500; //Ping间隔设置为500毫秒，用于检测与DDS代理之间的连接状态，确保通信链路稳定。
...
const AP_Param::GroupInfo AP_DDS_Client::var_info[] {   /////参数组定义
    ...
    // @RebootRequired: True
    // @User: Standard
    AP_GROUPINFO("_UDP_PORT", 2, AP_DDS_Client, udp.port, 2019),  ///UDP配置参数
    ...
    // @RebootRequired: True
    // @User: Standard
    AP_GROUPINFO("_DOMAIN_ID", 4, AP_DDS_Client, domain_id, 0), //设置DDS DOMAIN ID

    AP_GROUPEND
};

static void initialize(geometry_msgs_msg_Quaternion& q)  //四元数初始化
{
    q.x = 0.0;
    q.y = 0.0;
    q.z = 0.0;
...
AP_DDS_Client::~AP_DDS_Client() //析构函数，释放内存
{
    // close transport
    if (is_using_serial) {
...
void AP_DDS_Client::update_topic(builtin_interfaces_msg_Time& msg)  //更新时间消息
{
    uint64_t utc_usec;
    if (!AP::rtc().get_utc_usec(utc_usec)) {
        utc_usec = AP_HAL::micros64();
    }
    msg.sec = utc_usec / 1000000ULL;      ///msg是引用的一个builtin_interfaces_msg_Time类消息类型，而.sec是这个消息类中的一个实例
...
void AP_DDS_Client::populate_static_transforms(tf2_msgs_msg_TFMessage& msg) ///静态变换
{
...
void AP_DDS_Client::update_topic(sensor_msgs_msg_BatteryState& msg, const uint8_t instance) //电池状态话题更新
{
