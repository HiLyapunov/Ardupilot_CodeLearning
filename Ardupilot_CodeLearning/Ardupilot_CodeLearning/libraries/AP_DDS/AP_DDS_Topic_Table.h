...
#include "std_msgs/msg/Float32.h"  //添加float消息类型
...
static inline constexpr uint8_t to_underlying(const TopicIndex index) //内联函数，用于将 `TopicIndex` 枚举类型转换为其底层类型（`uint8_t`）。
{
...
constexpr struct AP_DDS_Client::Topic_table AP_DDS_Client::topics[] = {
    {
        .topic_id = to_underlying(TopicIndex::TIME_PUB), //为每个话题分配的唯一标识符。通过调用 `to_underlying()` 将枚举值 **`TIME_PUB`** 转换为其底层的整数值（`uint8_t`）
        .pub_id = to_underlying(TopicIndex::TIME_PUB),
        .sub_id = to_underlying(TopicIndex::TIME_PUB),  // **`pub_id`** 和 **`sub_id`**：用于标识发布者和订阅者的 ID。在这个例子中，**`TIME_PUB`** 被用作话题的发布者和订阅者。
        //Micro XRCE-DDS 使用的对象 ID 结构体，用于标识不同的 **数据写入器（DataWriter）和读取器（DataReader）**
        .dw_id = uxrObjectId{.id=to_underlying(TopicIndex::TIME_PUB), .type=UXR_DATAWRITER_ID}, //数据写入器的 ID，`type` 指定为 **`UXR_DATAWRITER_ID`**
        .dr_id = uxrObjectId{.id=to_underlying(TopicIndex::TIME_PUB), .type=UXR_DATAREADER_ID}, //数据读取器的 ID，`type` 指定为 **`UXR_DATAREADER_ID
        .topic_rw = Topic_rw::DataWriter,  ///////指定当前话题的模式。在这里是 **DataWriter**，表示这个话题是发布publish模式（用于写入数据）。
        .topic_name = "rt/ap/time",  //DDS 系统中的话题名称
        .type_name = "builtin_interfaces::msg::dds_::Time_", //话题的数据类型
        .qos = {                       //////////////QoS服务质量设置
