//引入头文件和声明位置控制功能所需的依赖模块
#include <AP_HAL/AP_HAL.h> //引入硬件抽象层头文件
#include "AC_PosControl.h" //引入位置控制头文件
#include <AP_Math/AP_Math.h> //引入数学运算头文件
#include <AP_Logger/AP_Logger.h> //引入日志系统头文件
#include <AP_Motors/AP_Motors.h>    //引入电机控制库头文件
#include <AP_Vehicle/AP_Vehicle_Type.h> //引入飞行器类型头文件
#include <AP_Scheduler/AP_Scheduler.h> //引入调度器头文件

extern const AP_HAL::HAL& hal;
//extern 关键字用于声明一个全局变量或函数，而这个变量或函数是在其他文件中定义的。它告诉编译器，
//该变量已经在其他地方定义，不需要在当前文件中重新定义，只需引用即可。通过 extern，多个源文件可以共享同一个变量。

#if APM_BUILD_TYPE(APM_BUILD_ArduPlane) //通过 #if 预处理指令，这些设置会根据构建时选择的飞行器类型而生效。
 ...
 # define POSCONTROL_VEL_XY_FILT_D_HZ           5.0f    // horizontal velocity controller input filter for D
#else
 // default gains for Copter / TradHeli //旋翼机默认增益
 # define POSCONTROL_POS_Z_P                    1.0f    // vertical position controller P gain default// 垂直位置控制器的比例增益默认值
 # define POSCONTROL_VEL_Z_P                    5.0f    // vertical velocity controller P gain default// 垂直速度控制器的比例增益默认值
 # define POSCONTROL_VEL_Z_IMAX                 1000.0f // vertical velocity controller IMAX gain default// 垂直速度控制器的积分最大输出限制默认值
 # define POSCONTROL_VEL_Z_FILT_HZ              5.0f    // vertical velocity controller input filter// 垂直速度控制器输入的滤波器频率
 # define POSCONTROL_VEL_Z_FILT_D_HZ            5.0f    // vertical velocity controller input filter for D// 垂直速度控制器微分项的输入滤波器频率
 # define POSCONTROL_ACC_Z_P                    0.5f    // vertical acceleration controller P gain default// 垂直加速度控制器的比例增益默认值
 # define POSCONTROL_ACC_Z_I                    1.0f    // vertical acceleration controller I gain default// 垂直加速度控制器的积分增益默认值
 # define POSCONTROL_ACC_Z_D                    0.0f    // vertical acceleration controller D gain default// 垂直加速度控制器的微分增益默认值
 # define POSCONTROL_ACC_Z_IMAX                 800     // vertical acceleration controller IMAX gain default// 垂直加速度控制器的积分最大输出限制默认值
 # define POSCONTROL_ACC_Z_FILT_HZ              20.0f   // vertical acceleration controller input filter default// 垂直加速度控制器输入的滤波器频率默认值
 # define POSCONTROL_ACC_Z_DT                   0.0025f // vertical acceleration controller dt default// 垂直加速度控制器的默认时间步长
 # define POSCONTROL_POS_XY_P                   1.0f    // horizontal position controller P gain default// 水平位置控制器的比例增益默认值
 # define POSCONTROL_VEL_XY_P                   2.0f    // horizontal velocity controller P gain default// 水平速度控制器的比例增益默认值
 # define POSCONTROL_VEL_XY_I                   1.0f    // horizontal velocity controller I gain default// 水平速度控制器的积分增益默认值
 # define POSCONTROL_VEL_XY_D                   0.5f    // horizontal velocity controller D gain default// 水平速度控制器的微分增益默认值
 # define POSCONTROL_VEL_XY_IMAX                1000.0f // horizontal velocity controller IMAX gain default// 水平速度控制器的积分最大输出限制默认值
 # define POSCONTROL_VEL_XY_FILT_HZ             5.0f    // horizontal velocity controller input filter// 水平速度控制器输入的滤波器频率
 # define POSCONTROL_VEL_XY_FILT_D_HZ           5.0f    // horizontal velocity controller input filter for D// 水平速度控制器微分项的输入滤波器频率
#endif

// vibration compensation gains//振动补偿增益
